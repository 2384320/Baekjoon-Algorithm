    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i == 0) {
                ...
                int preMaxDp0 = maxDp[0];
                int preMaxDp2 = maxDp[2];
                maxDp[0] = Math.max(maxDp[0], maxDp[1]) + x1;
                maxDp[2] = Math.max(maxDp[1], maxDp[2]) + x3;
                maxDp[1] = Math.max(maxDp[1], Math.max(preMaxDp0, preMaxDp2)) + x2;

                int preMinDp0 = minDp[0];
                int preMinDp2 = minDp[2];
                minDp[0] = Math.min(minDp[0], minDp[1]) + x1;
                minDp[2] = Math.min(minDp[1], minDp[2]) + x3;
                minDp[1] = Math.min(minDp[1], Math.min(preMinDp0, preMinDp2)) + x2;
            }
        }
